import {getIsIframe} from "@arcnight/shared";
...
const useFocusTrap = ({
  ...
  useEffect(() => {
    ...
    const focusElement = (
      node?: FocusableElement | HTMLElement | null,
    ...
    const contain = (root: HTMLDivElement) => {
      ...
      const isFocusedElementOutsideFrame =
        root.ownerDocument.body === document.activeElement && isIframe;
    ...
    return () => {
      ...
      const isFocusInAnotherRoot =
        targetRootElement && targetRootElement !== rootRef.current;
      const isFocusInThisRoot =
        targetRootElement && targetRootElement === rootRef.current;
      const isCurrentActiveElementFocusable =
        currentActiveElement && isFocusable(currentActiveElement);
